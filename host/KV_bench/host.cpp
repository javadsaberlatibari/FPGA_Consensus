...
void run_user_kernel(bool initilize, cl::Context context, cl::CommandQueue q, cl::Kernel user_kernel, 
                     uint32_t nOP, uint32_t wP, uint32_t node_id, uint32_t N_node, cl::Buffer buffer_r1) {
    cl_int err;
...
int main(int argc, char **argv) {
    ...
    run_user_kernel(true, context, q, user_kernel, 99999, 100, node_id, N_node, buffer_r1);
    if(ycsb_mode=="Update_Heavy"){
        bool initilize=false;
        run_user_kernel(initilize, context, q, user_kernel, nOP, 50, node_id, N_node, buffer_r1);
    }
    else if(ycsb_mode=="Read_Mostly"){
        bool initilize=false;
        run_user_kernel(initilize, context, q, user_kernel, nOP, 5, node_id, N_node, buffer_r1);
    }
    else if(ycsb_mode=="Read_Only"){
        bool initilize=false;
        run_user_kernel(initilize, context, q, user_kernel, nOP, 0, node_id, N_node, buffer_r1);
    }
