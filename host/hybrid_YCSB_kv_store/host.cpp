...
const int expected_value=999999;
const int shared_mem_size= 1000000;
...
void prepare_operations_and_transfer(bool initilize, bool local, cl::Context context, cl::CommandQueue q,
    ...
    if (((FPGAnOP * wP) / 100) > 2) {
        num_writes = ((FPGAnOP * wP) / 100) - 2;
    }

    std::vector<int> all_indices;
    for (int i = 1; i < FPGAnOP - 1; ++i) {
    ...
    fpgaoperations[0] = (uint64_t(1) << 32) | 1;
    fpgaoperations[FPGAnOP - 1] = (uint64_t(1) << 32) | 1;
    //fpgaoperations[FPGAnOP - 1] = (uint64_t(expected_value) << 32) | expected_value;
    ...
    if(!initilize){
        ...
        std::uniform_int_distribution<> cpu_key_dist(100000, 999998);
        ...
        if (((CPUnOP * wP) / 100) > 2) {
        cpu_num_writes = ((CPUnOP * wP) / 100) - 2;
        }

        std::vector<int> cpu_all_indices;
        for (int i = 1; i < CPUnOP - 1; ++i) {
        ...
        //cpu_operations[0] = (uint64_t(100000) << 32) | 100000;
        //cpu_operations[CPUnOP - 1] = (uint64_t(100000) << 32) | 100000;
    ...
    cl::Buffer buffer_fpgaoperations(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
    ...
    OCL_CHECK(err, err = user_kernel.setArg(13, check_value));
    //OCL_CHECK(err, err = user_kernel.setArg(13, check_value_cpu));
    OCL_CHECK(err, err = user_kernel.setArg(14, local));
    OCL_CHECK(err, err = user_kernel.setArg(15, buffer_fpgaoperations)); // ✅ host-pinned
    OCL_CHECK(err, err = user_kernel.setArg(16, buffer_r1));
    OCL_CHECK(err, err = user_kernel.setArg(17, buffer_shared_data));
...
void run_user_kernel(bool initilize, cl::Context context, cl::CommandQueue q, cl::Kernel user_kernel, cl::Kernel hybrid_kernel,
            ...
            if(value==0){
                ...
                if(summarization_count< summariztion_period){
                    ...
                    if(1<node_num){
                        ...
                        OCL_CHECK(err, hybrid_kernel.setArg(0, ((uint64_t)(summarized_kv_array[key].value & 0xFFFFFFFF) << 32) | ((uint64_t)(key & 0xFFFFF) << 12) | (shared_data_time_stamp & 0xFFF)));
    ...
    OCL_CHECK(err, hybrid_kernel.setArg(0, ((uint64_t)(1 & 0xFFFFFFFF) << 32) | ((uint64_t)(1 & 0xFFFFF) << 12) | (1 & 0xFFF)));
    OCL_CHECK(err, q.enqueueTask(hybrid_kernel)); //set check value and send to FPGA. 
    OCL_CHECK(err, hybrid_kernel.setArg(0, ((uint64_t)(1 & 0xFFFFFFFF) << 32) | ((uint64_t)(expected_value & 0xFFFFF) << 12) | (1 & 0xFFF)));
    OCL_CHECK(err, q.enqueueTask(hybrid_kernel)); //set check value and send to FPGA. 
...
int main(int argc, char **argv) {
    ...
    prepare_operations_and_transfer(true, true, context, q, 100000, 100, user_kernel, buffer_r1, node_id, N_node);
    run_user_kernel(true, context, q, user_kernel, hybrid_kernel, nOP, buffer_r1, N_node);
    ...
    if(ycsb_mode=="Update_Heavy"){
        ...
        prepare_operations_and_transfer(false, local, context, q, nOP, 50, user_kernel, buffer_r1, node_id, N_node);
    }
    else if(ycsb_mode=="Read_Mostly"){
        bool local=true; //To measure response time this one should be true, so don't wait for remote latencies.
        if(std::strcmp(argv[6], "th") == 0)
            local=false;
        prepare_operations_and_transfer(false, local, context, q, nOP, 5, user_kernel, buffer_r1, node_id, N_node);
    }
    else if(ycsb_mode=="Read_Only"){
        bool local=true;
        prepare_operations_and_transfer(false, local, context, q, nOP, 0, user_kernel, buffer_r1, node_id, N_node);
    }
...
/*=================MEMCACHE SYNC END===============================*/
    ...
    //sleep(15);
    run_user_kernel(false, context, q, user_kernel, hybrid_kernel, nOP, buffer_r1, N_node);
