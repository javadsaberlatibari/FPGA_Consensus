...
# 423 "/home/pyuvaraj/fpga-rdma-testbed/kernel/user_krnl/project_krnl/src/hls/project_krnl.cpp"
}
...
#pragma HLS STREAM depth=64 variable=ncc_tx_data
 ...
 static int counter = 0;
    ...
    VITIS_LOOP_553_2: while(counter < num_ops) {
        debug_counter++;
        if (read_op) {
            switch(ops[counter]) {
                ...
                case 2: {
                    ...
                    if (!proposed.full()){
                        if (employee[(int) temp.range(31, 16)] && projects[(int) temp.range(15, 0)]) {
                            val.range(31, 30) = 2;
                ...
                case 3: {
                    if (!broadcast_req.full() && !update_set_request.full()) {
                        ...
                        if (!employee[(int) amount[counter]]) {
                            broadcast_req.write(amount[counter]);
                            employee[amount[counter]] = true;
                            employee_size++;
                ...
                case 4: {
                    if (!query_mem_req.full()) {
                        query_mem_req.write(project_size + assign_size);
                        counter++;
                    }
                    break;
                }
        ...
        if (!smr_updated.empty()) {
            ...
            switch(temp.range(31, 30)) {
                case 0: {
                    projects[(int) temp.range(29, 0)] = true;
                    project_size += 1;
                }

                case 1: {
                    projects[(int) temp.range(29, 0)] = false;
                    project_size -= 1;
                }

                case 2: {
                    assign[assign_counter] = temp;
                    assign_size+=1;
                    assign_counter++;
                }
            }
...
#pragma HLS INTERFACE axis port = update_set_request
 ...
 static int bram_counter=0;
    ...
    static bool employee[2000];
    VITIS_LOOP_750_1: for (int i = 0; i < 1000; i++) {
        employee[i] = true;
    }
    ...
    VITIS_LOOP_760_2: while(query_cnt<query_num) {
        exe_cnt++;
        internal_clock++;
        ...
        if (!update_set_request.empty()) {
            ...
            if (!employee[(int) temp]) {
                employee[(int) temp] = true;
                tmp_local_counter++;
            }
        }
        ...
        if (!minPropReadBram_req.empty() && !minPropReadBram_rsp.full()) {
            ...
            VITIS_LOOP_787_3: for (int i = 0; i < node_num-1; i++) {
        ...
        if (!readSlotsReadBram_req.empty() && !readSlotsReadBram_rsp.full()) {
            ...
            maxPropNumber = 0;
            VITIS_LOOP_801_4: for (int i = 0; i < node_num-1; i++) {
                if (FOLLOWER_LIST[i]) {
        ...
        if(internal_clock==update_period){
            ...
            VITIS_LOOP_836_5: for (int i=0; i<node_num; i++){
                if(i!=board_num){
    ...
    // VITIS_LOOP_859_6: while (query_cnt == query_num && board_num != 0 && !fin_sig_found) {
