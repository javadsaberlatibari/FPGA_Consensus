    ...
    while(counter < num_ops) {
        if (read_op) {
            switch(ops[counter]) {
                ...
                //Delete Movie (Always permissible)
                case 1: {
                    if (!proposed.full()) {
                        ap_uint<32> val; 
                        val.range(31, 30) = 1;
                ...
                //Add Customer (Always permissible)
                case 2: {
                    if (!proposed.full()) {
                        ap_uint<32> val; 
                        val.range(31, 30) = 2;
                        val.range(30, 0) = amount[counter];
                        proposed.write(ProposedValue(val, 1));
                        read_op = false; 
                    }
                    break; 
                }

                //Delete Customer (Always permissible)
                case 3: {
                    if (!proposed.full()) {
                        ap_uint<32> val; 
                        val.range(31, 30) = 3;
                ...
                case 4: {
                    if (!query_mem_req.full()) {
                        query_mem_req.write(customer_size + movie_size);
                        counter++; 
                    }
                    break; 
                }
        ...
        if (!smr_updated.empty()) {
            ...
            switch(temp.range(31, 30)) {
                case 0: {
                    movies[temp.range(30, 0)] = true;
                    movie_size += 1;
                }

                case 1: {
                    movies[temp.range(30, 0)] = false;
                    movie_size -= 1;
                }

                case 2: {
                    customers[temp.range(30, 0)] = true;
                    customer_size += 1;
                }

                case 3: {
                    customers[temp.range(30, 0)] = false;
                    customer_size -= 1;
                }
            }
        }
