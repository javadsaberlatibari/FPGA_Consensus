...
static int old_leader = 0, new_leader = 0; 
static int LEADER = 0; 
...
static bool FOLLOWER_LIST[NUM_NODES] = {1, 1, 1, 1};
...
const int FIN_SIG_PTR = 500000;
const int FIN_SIG_ADDR = FIN_SIG_PTR * 4; 
...
void meta_converter(hls::stream<ap_uint<256>>& a_tx_meta, hls::stream<pkt256>& b_tx_meta) {
    ...
    if (!a_tx_meta.empty()) {
        ...
        b_tx_meta.write(temp_pkt_256);

    }
}

void data_converter(hls::stream<ap_uint<64>>& a_tx_data, hls::stream<pkt64>& b_tx_data) {
    #pragma HLS inline off
    #pragma HLS pipeline II=1
    ...
    if (!a_tx_data.empty()) {
        ...
        temp_pkt_64.last = 1; 
        b_tx_data.write(temp_pkt_64);
    }    
}

void meta_merger_to_end(
    ...
    if (!a_tx_meta.empty() && !c_tx_meta.full()) {
        a_tx_meta.read(temp_val_256);
        temp_pkt_256.data(255, 0) = temp_val_256.range(255, 0); 
...
void meta_merger(
    ...
    } else if (!b_tx_meta.empty() && !c_tx_meta.full()) {
...
void data_merger(
    ...
    if (!a_tx_data.empty() && !c_tx_data.full()) {
        a_tx_data.read(temp_val_64);
        temp_pkt_64.data(63, 0) = temp_val_64.range(63, 0); 
        temp_pkt_64.keep(7, 0) = 0xff;
        temp_pkt_64.last = 1; 
        c_tx_data.write(temp_pkt_64);
    } else if (!b_tx_data.empty() && !c_tx_data.full()) {
