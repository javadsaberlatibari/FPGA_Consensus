...
void leader_switch(
    ...
    if (counter % 100000 == 0 && !hb_scan.full()) {
        counter = 0; 
        int j=0; 
...
void log_handler_fsm(
    ...
    hls::stream<ap_uint<32>>& alive_update,
    int myBoardNum,
    int num_nodes
    //int* reply
) {
    ...
    static bool ALIVE[NUM_NODES] = {1, 1, 1, 1};


    if (!alive_update.empty()) {
        ap_uint<32> node;
        alive_update.read(node);
...
void smr(
    ...
    if (!updateLocalValue_req.empty() && !smr_finished.full()) {
...
void crdt_counter(
    ...
    if (!broadcast_req.empty() && !ncc_finished.full()) {
    ...
    if (!ncc_permissible_req.empty() && !ncc_permissible_rsp.full()) {
...
void bank(
    ...
    static hls::stream<ap_uint<256>> hb_tx_meta; 
    ...
    #pragma HLS STREAM depth=128 variable=hb_tx_meta

    static int counter = 0; 
    static int debug_counter = 0; 
    static bool read_op = true; 
    static int inital_value = 100000; 
    static hls::stream<ap_uint<32>> alive_smr, alive_crdt; 
    #pragma HLS STREAM depth=8 variable=alive_smr
    #pragma HLS STREAM depth=8 variable=alive_crdt
    static ap_uint<32> leader = 0; 

    //while(counter < num_ops && debug_counter < debug_exe) {
    while(counter < num_ops) {
        ...
        if (read_op) {
            switch(ops[counter]) {
                ...
                case 0: {
                    if (!smr_permissble_req.full() && !ncc_permissible_req.full() && leader == myBoardNum) {
...
//###########################################################
        if (!alive_update.empty() && !alive_smr.full() && !alive_crdt.full()) {
            ap_uint<32> temp; 
            alive_update.read(temp);
            alive_smr.write(temp);
            alive_crdt.write(temp);
        }

        if (!leader_update.empty()) {
            leader_update.read(leader);
        }
//###########################################################
        smr(
            smr_finished,
...
//###########################################################
        ...
        if (!smr_permissible_rsp.empty() && !ncc_permissible_rsp.empty() && !proposed.full()) {
...
void mem_manager(  
    ...
    static ap_uint<32> heartbeat_clock=0; 
    ...
    //for (int i = 0; i < NUM_NODES; i++) ALIVE[i] = 1;

    static int hb_values[NUM_NODES];
    static int hb_scores[NUM_NODES] = {5, 5, 5, 5};
    static bool local_alive[NUM_NODES] = {1, 1, 1, 1};
    static ap_uint<32> leader = 0; 
...
//###########################################################
        if (!hb_scan.empty()) {
            ...
            network_ptr[board_num] = hb;

            for (int i = 0; i < NUM_NODES; i++) {
                
                if (i != board_num) {
                    int temp = network_ptr[i];
                    if (temp == hb_values[i]) {
                        hb_scores[i]--;
                    } else {
                        hb_scores[i]++;
            ...
            while (j<NUM_NODES){
                if (j != board_num) {
                    if(hb_scores[j] <= 2 && local_alive[j] ) {
                        //ALIVE[j] = 0;
                        ...
                        new_state.last = 1; 
                        if(!permission_switch.full() && !alive_update.full()) { 
                            permission_switch.write(new_state);
                            alive_update.write(j);
                            if (j == leader) {
                                for (int i = 0; i < NUM_NODES; i++) {
                                    if (local_alive[i]) {
                                        leader_update.write(i);
                                        break; 
                                    }
                                }
                            }
                    ...
                    } else  if (hb_scores[j] >= 6 && !local_alive[j] ) {
                        //ALIVE[j] = 1;
                        
                        local_alive[j] = 1;
                        new_state.data(15, 0) = qpn_tmp;//qp_num
                        new_state.data(31, 16) = 2;//new_state RTR
                        new_state.keep(7, 0) = 0xff;
                        new_state.last = 1; 
                        if(!permission_switch.full() && !alive_update.full()) { 
                            alive_update.write(j);
...
void bank_account_krnl(
    ...
    static hls::stream<int> hb_scan;
    static hls::stream<ap_uint<32>> alive_update;
    static hls::stream<ap_uint<32>> leader_update;
    ...
    #pragma HLS STREAM depth=8 variable=hb_scan
    #pragma HLS STREAM depth=8 variable=alive_update
    #pragma HLS STREAM depth=8 variable=leader_update
