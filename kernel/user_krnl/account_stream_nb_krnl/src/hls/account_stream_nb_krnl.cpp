#include "account_stream_nb_krnl.h"
...
void mem_manager( 
    ...
    hls::stream<int>& update_noncon_req,
    ...
    while (running) {
        ...
        //if (internal_clock == 10000) {
            //deposit
        if (!update_noncon_req.empty()) {

            update_noncon_req.read(update_req);
            if (!update_noncon_rsp.full()) {
                ...
                hbm_tmp.range(63, 32) = update_req; 
    ...
    */
    ...
    BANK_MAIN_LOOP: while (counter < number_of_operations || withdraw_updates < debug_exe) {
        ...
        if (!update_noncon_rsp.empty()) {
            ...
            remote_bank_accounts = update_noncon.range(31, 0);

            switch(update_noncon.range(63, 32)) {
...
void account_stream_nb_krnl(
    ...
    static hls::stream<int> update_noncon_req;
    ...
    #pragma HLS STREAM depth=64 variable=update_noncon_req
    #pragma HLS STREAM depth=64 variable=update_noncon_rsp
