...
void mem_maneger(int *network_ptr, int node_num, int board_num, int query_num, int check_value, hls::stream<ap_uint<64> >& axis_mem_request){
    #pragma HLS PIPELINE II=1
    ...
    while ((query_cnt<query_num) || (check_throughput_count!=(node_num-1))){
        ...
        if(internal_clock==update_period){
            ...
            remote_counter=0;
                    ...
                    mem_counter = remote_counter + local_counter;
        }
    }
}
...
extern "C" {
    void hbm_counter_bram_bench(
        ...
        mem_maneger(network_ptr, node_num, board_num, query_num, check_value, axis_mem_request);
    }
}
