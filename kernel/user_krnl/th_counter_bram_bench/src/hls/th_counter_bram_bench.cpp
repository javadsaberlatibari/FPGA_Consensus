...
void bram_mem_maneger(int *network_ptr, int node_num, int board_num, int query_num, int check_value, hls::stream<ap_uint<64> >& axis_mem_request){
    #pragma HLS PIPELINE II=1
    static int bram_counter=0;
    static int update_period=10000;
...
extern "C" {
    void th_counter_bram_bench(
        ...
        bram_mem_maneger(network_ptr, node_num, board_num, query_num, check_value, axis_mem_request);
    }
}
