...
void process_kv(
    ...
    hls::stream<ap_uint<64>>& update_requests_shared_mem_local,
    ...
    while (op_cnt < op_num || (finish_signal_process_kv.empty()) /*|| (check_value_met_cpu!=(node_num-1)&& !init)*/) {
        switch(state) {
            case FETCH_OP:
    ...
    finish_signal_shared_mem.write(1111);
}

void hbm_remote_handler(int write_num, int init, int update_period, int key_num_bits, int expected_packets, int node_num, int board_num, int *network_ptr, hls::stream<ap_uint<64>>& update_requests_bram_mem, hls::stream<ap_uint<64>>& update_requests_shared_mem, hls::stream<ap_uint<64>>& finish_signal_process_kv){
    #pragma HLS PIPELINE II=1
                ...
                if (i != board_num) {
                    ...
                    if(update_data!=0){
                        ...
                        if (key < NUM_KEYS) {
                            ...
                            update_requests_shared_mem.write(update_data);
                        }
                    }
                }
            }
...
void update_shared_memory(int key_num_bits, hls::stream<ap_uint<64>>& update_requests_local, hls::stream<ap_uint<64>>& update_requests_remote, ap_uint<64>* shared_data, hls::stream<ap_uint<64>>& finish_signal_shared_mem){
    #pragma HLS PIPELINE II=1
    ap_uint<64> tmp;
    int key_bit_start = 32- key_num_bits;
    while(finish_signal_shared_mem.empty()){
        if(!update_requests_local.empty()){
            ap_uint<64> data;
        
            update_requests_local.read(data);
        ...
        if(!update_requests_remote.empty()){
            ap_uint<64> data;
        
            update_requests_remote.read(data);
            ap_uint<16> timestamp = data.range(key_bit_start-1, 0);
            ap_uint<32> key = data.range(31, key_bit_start);
            ap_uint<32> value = data.range(63, 32);
    ...
    finish_signal_shared_mem.read(tmp);
}
...
extern "C" {
    void hybrid_kv_store_summarizing(
        ...
        hls::stream<ap_uint<64>> update_requests_bram_mem_remote; 
        hls::stream<ap_uint<64>> update_requests_shared_mem_local; 
        hls::stream<ap_uint<64>> update_requests_shared_mem_remote; 
        hls::stream<ap_uint<64>> finish_signal_process_kv;
        hls::stream<ap_uint<64>> finish_signal_shared_mem;
        ...
        #pragma HLS STREAM depth=8 variable=update_requests_shared_mem_local
        #pragma HLS STREAM depth=8 variable=update_requests_shared_mem_remote
        ...
        process_kv(
            ...
            update_requests_shared_mem_local,
        ...
        hbm_remote_handler(
            ...
            network_ptr, 
            update_requests_bram_mem_remote, 
            update_requests_shared_mem_remote, 
            finish_signal_process_kv);
