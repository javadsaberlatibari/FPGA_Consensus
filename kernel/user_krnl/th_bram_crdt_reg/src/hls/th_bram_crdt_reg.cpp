...
void bram_mem_maneger_lww(int *network_ptr, int *reg, int node_num, int board_num, int check_value, int query_num, hls::stream<ap_uint<32> >& axis_mem_request, hls::stream<ap_uint<16> >& now_tmie_stamp){
        ...
        if(internal_clock==update_period){
            internal_clock=0;
                    for (int i=0; i<node_num; i++){
                        ...
                        if(!check_finish[i]){
                                if(tmp_reg.range(15,0)==check_value){
                                    check_finish[i]= true;
...
extern "C" {
    ...
    void th_bram_crdt_reg(
        ...
        static hls::stream<ap_uint<16> > now_tmie_stamp;
            ...
            bram_mem_maneger_lww(network_ptr, crdt, node_num, board_num, check_value, query_num, axis_mem_request,now_tmie_stamp);
    }
