...
void mem_manager( 
    ...
    hls::stream<int>& memory_manager_meta,
    hls::stream<int>& leader_update,
    ...
    #pragma HLS INTERFACE axis port = memory_manager_meta
    ...
    while (running) {
        ...
        network_ptr[HB_BASE_PTR + board_number] = hb;
        if (hb % 1000 && !scan) {
            scan = true; 
            if (!memory_manager_meta.full()) {
                memory_manager_meta.write(hb);
            }
            counter = 0;
        }
        ...
        if (counter == 100) {
            ...
            //memory_manager_meta.write(hb);
            //Update Local HB
            int qpn_tmp=board_number*(NUM_NODES-1);
            for (int i = 0; i < number_of_nodes; i++) {
                if (i != board_number) {
                    if (network_ptr[i] != HB_values[i]) {
                        HB_thresholds[i]++;
                    } else {
                        HB_thresholds[i]--;
                    }
                    ...
                    if (HB_thresholds[i] < 2 && HB_Status[i]) {
                        HB_Status[i] = false; 
                        std::cout << "Remote Node " << i << " crashed." << std::endl; 
                        new_state.data(15, 0) = qpn_tmp + i;//qp_num
                        new_state.data(31, 16) = 0;//new_state init
                        new_state.keep(7, 0) = 0xff;
                        new_state.last = 1; 
                        permission_switch.write(new_state);

                        if (i == leader && !leader_update.full()) {
                            leader = leader + 1; 
                            leader_update.write(leader);
                        }
                    } else if (HB_thresholds[i] >= 7 && !HB_Status[i]) {
                        HB_Status[i] = true; 
        ...
        // while (j<NUM_NODES){
        //     if (j != board_number) {
        //         if (HB_thresholds[j] < 2 && HB_Status[j]) {
        //             HB_Status[j] = false; 
        //             std::cout << "Remote Node " << j << " crashed." << std::endl; 
        //             new_state.data(15, 0) = qpn_tmp;//qp_num
        //             new_state.data(31, 16) = 0;//new_state init
        //             new_state.keep(7, 0) = 0xff;
        //             new_state.last = 1; 
        ...
        //                 qpn_tmp++;
        //             }
        //             if (j == leader && !leader_update.full()) {
        //                 leader = leader + 1; 
        //                 leader_update.write(leader);
        //             }

        //         } else if (HB_thresholds[j] >= 7 && !HB_Status[j]) {
        //             HB_Status[j] = true; 
        //             std::cout << "Remote Node " << j << " un-crashed." << std::endl; 
        //             new_state.data(15, 0) = qpn_tmp;//qp_num
        //             new_state.data(31, 16) = 2;//new_state init
        //             new_state.keep(7, 0) = 0xff;
        //             new_state.last = 1; 
...
void bank(
    ...
    */
    ...
    static hls::stream<ap_uint<256>> hb_meta;
    #pragma HLS STREAM depth=8 variable=hb_meta
    ...
    */
    ...
    BANK_MAIN_LOOP: while (counter < number_of_operations || withdraw_updates < debug_exe) {
        ...
        if (!memory_manager_meta.empty()) {
            memory_manager_meta.read(hb);
            int j=0;
            int qpn_tmp=board_number*(number_of_nodes-1);
...
void account_stream_krnl(
    ...
    static hls::stream<int> memory_manager_meta;
    #pragma HLS STREAM depth=8 variable=memory_manager_meta
    static hls::stream<int> leader_update;
