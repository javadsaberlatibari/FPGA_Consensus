...
void leader_switch(
    ...
    if (!hb_increment.full() && counter % 1000 == 0) {
        counter = 1; 
        static int hb = 0;
        hb++;
        int j=0;
            ...
            if(j!=board_number){
                if(!m_axis_tx_meta.full() && !m_axis_tx_data.full()) { 
                    rdma_write(
                        qpn_tmp,
                        0,
                        BROADCAST_BASE_ADDR + (4 * 2 * board_number),
                        0x8,
...
void mem_manager( 
    ...
    static int HB_thresholds[NUM_NODES] = {5, 5, 5, 5};
    static bool HB_Status[NUM_NODES] = {1, 1, 1, 1};
    internal_clock++;
    ...
    if (!hb_increment.empty()) {
        ...
        while (j<NUM_NODES){
            if (j != board_number) {
                if (HB_thresholds[j] < 5 && HB_Status[j]) {
                    HB_Status[j] = false; 
                ...
                } else if (HB_thresholds[j] >= 5 && !HB_Status[j]) {
                    HB_Status[j] = true; 
