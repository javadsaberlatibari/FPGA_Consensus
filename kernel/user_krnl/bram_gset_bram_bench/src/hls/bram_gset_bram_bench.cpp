...
void crdt_gset(
    ...
    hls::stream<ap_uint<64> >& set_size_request
    //int wait_cyc
) {
    //#pragma inline 
    //#pragma HLS dataflow
                ...
                if (!axis_last_op_response.empty()){
                    ...
                    if(last_operation == 0) {
                        ...
                        if(!set_size_request.empty())
                            set_size_request.read(set_size);
                        read_value= set_size;
                        op_cnt++;
                    } else {
...
void update_local_gset(int write_num, hls::stream<ap_uint<64> >& update_set_request, hls::stream<ap_uint<64> >& local_set_e, hls::stream<ap_uint<64> >& break_signal){
    ...
    static int local_gset [400000]={0};
...
void update_remote_gset(int *network_ptr, int node_num, int write_num, int board_num, int expected_packets, bool throughput, hls::stream<ap_uint<64> >& local_set_e, hls::stream<ap_uint<64> >& break_signal , hls::stream<ap_uint<64> >& set_size_request){
    ...
    while((!break_flag) || ((received_packet_count==expected_packets)&&throughput)){
        ...
        // Write the set size to the set_size_request stream
...
void bram_mem_maneger_gset(int *network_ptr, int node_num, int board_num, int query_num, int write_num, int expected_packets , bool throughput, hls::stream<ap_uint<64> >& update_set_request, hls::stream<ap_uint<64> >& set_size_request){

    //#pragma HLS PIPELINE II=1
    #pragma HLS DATAFLOW
    
    static hls::stream<ap_uint<64> > local_set_e, break_signal;
    #pragma HLS STREAM depth=8 variable=local_set_e

    update_local_gset(write_num, update_set_request, local_set_e, break_signal);
    update_remote_gset(network_ptr, node_num, write_num, board_num, expected_packets, throughput, local_set_e, break_signal , set_size_request);
...
extern "C" {
    ...
    void bram_gset_bram_bench(
        ...
        #pragma HLS DATAFLOW
            ...
            bram_mem_maneger_gset(network_ptr, node_num, board_num, query_num, write_num, expected_packets, throughput, update_set_request, set_size_request);
    }
