...
void crdt_twopset(
    ...
    hls::stream<pkt64>& m_axis_tx_data,
    hls::stream<ap_int<64> >& axis_last_op_response,
    hls::stream<ap_int<64> >& add_set_size_request,
    hls::stream<ap_int<64> >& remove_set_size_request,
    ...
    int log_size= write_num +5;
    ap_int<64> add_set_size=0;
    ap_int<64> remove_set_size=0;
    ap_int<64> read_value;
            ...
            case OPERATION_FETCH:
                if (!axis_last_op_response.empty()){
                    ...
                    if(last_operation == 0) {
                        ...
                        if(!add_set_size_request.empty())
                            add_set_size_request.read(add_set_size);
                        if(!remove_set_size_request.empty())
                            remove_set_size_request.read(remove_set_size);
                        read_value= add_set_size-remove_set_size;
                        op_cnt++;
                    } else {
...
void update_local_twopset(int write_num, hls::stream<ap_int<64> >& update_set_request, hls::stream<ap_int<64> >& local_set_e_addset, hls::stream<ap_int<64> >& local_set_e_removeset, hls::stream<ap_int<64> >& break_signal_twopset){
...
void update_remote_twopset(int *network_ptr, int node_num, int board_num, int write_num, int expected_packets, bool throughput, hls::stream<ap_int<64> >& break_signal, hls::stream<ap_int<64> >& remote_set_e_addset, hls::stream<ap_int<64> >& remote_set_e_removeset, hls::stream<ap_int<64> >& break_signal_addset, hls::stream<ap_int<64> >& break_signal_removeset){
    ...
    while((!break_flag) || ((received_packet_count==expected_packets)&&throughput)){
        ...
        if(i!=board_num){
            tmp_read=network_ptr[((i*log_size)+log_index[i])*2];
            if(tmp_read!=0){
...
void bram_mem_maneger_twopset(int *network_ptr, int node_num, int board_num, int write_num, int query_num, int expected_packets, bool throughput, hls::stream<ap_int<64> >& add_set_size_request, hls::stream<ap_int<64> >& remove_set_size_request, hls::stream<ap_int<64> >& update_set_request){
    #pragma HLS DATAFLOW
    ...
    update_local_twopset(write_num, update_set_request, local_set_e_addset, local_set_e_removeset, break_signal_twopset);
    update_remote_twopset(network_ptr, node_num, board_num, write_num, expected_packets, throughput, break_signal_twopset, remote_set_e_addset, remote_set_e_removeset, break_signal_addset, break_signal_removeset);
    update_addset(remote_set_e_addset,local_set_e_addset, break_signal_addset , add_set_size_request);
    update_removeset(remote_set_e_removeset,local_set_e_removeset, break_signal_removeset , remove_set_size_request);
...
extern "C" {
    ...
    void bram_twopset_bram_bench(
        ...
        static hls::stream<ap_int<64> > add_set_size_request;
        static hls::stream<ap_int<64> > remove_set_size_request;
        ...
        #pragma HLS DATAFLOW
            ...
            crdt_twopset(
                ...
                m_axis_tx_data,
                axis_last_op_response,
                add_set_size_request,
                remove_set_size_request,
                update_set_request
                );
            bram_mem_maneger_twopset(network_ptr, node_num, board_num, write_num, query_num, expected_packets, throughput, add_set_size_request, remove_set_size_request, update_set_request);
    }
