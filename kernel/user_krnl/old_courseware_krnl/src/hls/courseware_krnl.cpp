    ...
    while(counter < num_ops) {
        if (read_op) {
            switch(ops[counter]) {
                ...
                //Delete Course (Always permissible)
                case 1: {
                    if (!proposed.full()) {
                        ap_uint<32> val; 
                        val.range(31, 30) = 0;
        ...
        if (!smr_updated.empty()) {
            ...
            switch(temp.range(31, 30)) {
                case 0: {
                    courses[temp.range(30, 0)] = true;
                    course_size += 1;
                }

                case 1: {
                    courses[temp.range(30, 0)] = false;
                    course_size -= 1;
                }
...
void mem_manager(  volatile int *network_ptr, 
    ...
    //&& exe_cnt < exe){
        ...
        if (!update_set_request.empty()) {
            ...
            if (!courrses[(int) temp]) {
                courrses[(int) temp] = true;
                tmp_local_counter++; 
            }
        }
