            ...
            if(j!=board_number){
                if(!m_axis_tx_meta.full() && !m_axis_tx_data.full()) { 
                    rdma_write(
                        qpn_tmp,
                        0,
                        BROADCAST_BASE_PTR + (4 * 2 * BROADCAST_SG_LEN * board_number +  pValue.range(31, 16)),
                        0x8,
            ...
            if(j!=board_number){
                if(!m_axis_tx_meta.full() && !m_axis_tx_data.full()) { 
                    rdma_write(
                        qpn_tmp,
                        0,
                        BROADCAST_BASE_ADDR + (4 * 2 * BROADCAST_SG_LEN * board_number + queue_slot),
                        0x8,
                        pValue,
                        m_axis_tx_meta, 
                        m_axis_tx_data
    ...
    */
    ...
    BANK_MAIN_LOOP: while (counter < number_of_operations || withdraw_updates < debug_exe) {
            ...
            switch (operation.range(31, 0))
            {
                ...
                case 1: {
                    ...
                    if (!proposed.full() && done) {
                        temp_amount = operation.range(63, 32);
                        if (directbuysell[temp_amount.range(31, 16)] - temp_amount.range(15, 0) >= 0) {
        ...
        if (!smr_updated.empty()) {
            ...
            {
            case 0:
                std::cout << "StoreBuyNow Method: Item: " << temp.range(29, 16) << " Stock: " << temp.range(15, 0) << std::endl; 
                directbuysell[temp.range(29, 16)] -= temp.range(15, 0);
                break;
            
            case 1:
                std::cout << "RegisterUser Method: User: " << temp.range(29, 0) << std::endl; 
                users[temp.range(29, 0)] = true; 
                break; 

            case 2:
                std::cout << "PlaceBid Method: Auction ID: " << temp.range(29, 21) << " User: " << temp.range(20, 12) << " Amount: " << temp.range(11, 0) << std::endl; 
                auctions[temp.range(29, 21)][1] = temp.range(20, 12);
                auctions[temp.range(29, 21)][2] = temp.range(11, 0);
                break;

            case 3:
                std::cout << "CloseAuction Method: Auction ID: " << temp.range(29, 0) << std::endl; 
                auctions[temp.range(29, 0)][0] = 2; 
                break; 
        ...
        if (!update_noncon_rsp.empty()) {
            ...
            if (update_noncon < 100 && update_noncon >= 0)
                auctions[update_noncon][0] = 1; 
        }
