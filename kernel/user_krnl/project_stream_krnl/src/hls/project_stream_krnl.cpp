...
void mem_manager( 
    ...
    if (internal_clock == 10000) {
        ...
        for (int i = 0; i < number_of_nodes; i++) {
            ...
            if (hbm_tmp != 0 && !update_noncon_rsp.full()) {
                hbm_bit = hbm_tmp;
                queue_slots[i]++;
...
void courseware(
    ...
    */
    ...
    COURSEWARE_MAIN_LOOP: while (counter < number_of_operations || withdraw_updates < debug_exe) {

        debug_counter++;
        std::cout << "Counter: " << counter <<  " Method: " << operation << std::endl; 
        if (done && !operation_stream.empty()) {
            ...
            switch (operation.range(31, 0))
            {
                ...
                case 2: {
                    //WorksOn
                    temp_amount = operation.range(63, 32);
                    if (courses[temp_amount.range(31, 16)] && students[temp_amount.range(15, 0)]) {
                ...
                case 4: {
                    //Query
                    std::cout << "Total: " << enroll_size << std::endl;
                    counter++; 
                    break;
                }
            }
...
void project_stream_krnl(
    ...
    #pragma HLS STREAM depth=8 variable=operation_stream
