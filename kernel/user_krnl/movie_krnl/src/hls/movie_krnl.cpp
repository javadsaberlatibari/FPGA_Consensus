...
void movie(
    ...
    */
    ...
    #else 
    while (counter < number_of_operations || (!throughput_finished) {
        ...
        if (done) {
            ...
            switch (operation_list[counter%BUFFER_SIZE])
            {
                ...
                case 3:
                {
                    std::cout << "Method: " << operation_list[counter%BUFFER_SIZE] << " Value: " << temp_amount.range(31, 0) << std::endl;
                    if (!permissibility_req.full()) {
                        temp_amount = amount_list[counter%BUFFER_SIZE];
                        proposed_value.range(31, 30) = operation_list[counter%BUFFER_SIZE]; 
