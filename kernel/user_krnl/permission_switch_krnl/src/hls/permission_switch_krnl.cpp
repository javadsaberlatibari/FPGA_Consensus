...
void bram_mem_maneger(int *network_ptr, int *counter, int exe_time, int node_num, int board_num, int query_num, hls::stream<ap_uint<256> >& axis_mem_request, hls::stream<pkt128>& permission_switch){
        ...
        if(internal_clock==update_period){
            ...
            for (int i=0; i<node_num; i++){
                if(i!=board_num){
                    ...
                    if(tmp_HBM !=0)
                        remote_counter = tmp_HBM;
                }
            }
            //bram_counter = remote_counter + local_counter;
            if(remote_counter>10000){
                if(one_time){
...
extern "C" {
    ...
    void permission_switch_krnl(
        ...
        static hls::stream<ap_uint<256> > axis_mem_request;
            ...
            bram_mem_maneger(network_ptr, crdt, exe_time, node_num, board_num, query_num, axis_mem_request, permission_switch);
