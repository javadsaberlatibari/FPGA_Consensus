    ...
    while(counter < num_ops) {
        if (read_op) {
            switch(ops[counter]) {
                ...
                //Add Project (Always permissible)
                case 0: {
                    if (!proposed.full()) {
                        ap_uint<32> val; 
                        val.range(31, 30) = 0;
                        val.range(30, 0) = amount[counter];
                ...
                //Delete Project (Always permissible)
                case 1: {
                    if (!proposed.full()) {
                        ap_uint<32> val; 
                        val.range(31, 30) = 0;
                ...
                //Assign (Ei and Si are valid)
                case 2: {
                    //Permissibility check
                    ap_uint<32> amount = amount[counter];
                    ap_uint<32> val; 

                    if (!proposed.full() && employee[amount.range(31, 16)] && projects[amount.range(15, 0)]){ 
                        val.range(31, 30) = 0;
                        val.range(30, 15) = amount.range(31, 16);
                        val.range(14, 0) = amount.range(15, 0);
                        proposed.write(ProposedValue(amount, 0));
                        read_op = false; 
                    }
                    break; 
                }
        ...
        if (!smr_updated.empty()) {
            //Perform totally ordered operation
            ap_uint<256> temp; 
            smr_updated.read(temp);
            switch(temp.range(31, 30)) {
                case 0: {
                    projects[temp.range(30, 0)] = true;
                    project_size += 1;
                }

                case 1: {
                    projects[temp.range(30, 0)] = false;
                    project_size -= 1;
                }
...
void mem_manager(  volatile int *network_ptr, 
    ...
    //&& exe_cnt < exe){
        ...
        if(!axis_mem_request.empty()){
            ...
            query_cnt++;
            reply[29] = tmp_local_counter + tmp_set_size;
