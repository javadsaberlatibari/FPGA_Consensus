...
void bram_mem_maneger(int *network_ptr, int node_num, int board_num, int query_num, hls::stream<ap_uint<64> >& axis_mem_request){
    #pragma HLS PIPELINE II=1
...
extern "C" {
    void bram_crdt_counter_bram_bench(
        hls::stream<pkt256>& m_axis_tx_meta,
        hls::stream<pkt64>& m_axis_tx_data,
        ...
        bram_mem_maneger(network_ptr, node_num, board_num, query_num, axis_mem_request);
    }
}
