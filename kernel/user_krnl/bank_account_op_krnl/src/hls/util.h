...
const int BROADCAST_BASE_PTR = HB_PTR_LEN + SYNC_GROUPS * LOG_PTR_LEN;
const int BROADCAST_BASE_ADDR = HB_ADDR_LEN + SYNC_GROUPS * LOG_ADDR_LEN;
...
struct LogEntry
{
    ...
    LogEntry(ap_uint<32> p, ap_uint<32> v, ap_uint<32> s, ap_uint<32> f) 
        :propVal(p), value(v), syncGroup(s), fuo(f) {}
