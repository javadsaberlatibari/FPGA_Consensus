...
for n in $Node_List; do
    ...
    #ssh pyuvaraj@pc${n}.cloudlab.umass.edu "source /opt/xilinx/xrt/setup.sh; ./host0 ${Usecase}.xclbin $NUM_Nodes $Number_of_Operations $Write_Percentage $h $EXE $IP > host${h}.log"&
    #ssh pyuvaraj@pc${n}.cloudlab.umass.edu "source /opt/xilinx/xrt/setup.sh; ./host0 network.xclbin $NUM_Nodes $Number_of_Operations $Write_Percentage $h $EXE > host${h}.log"&
    ssh pyuvaraj@pc${n}.cloudlab.umass.edu "source /opt/xilinx/xrt/setup.sh; ./host0 network.xclbin $NUM_Nodes $h $IP > host${h}.log"&
