...
Assign_FPGA="80"
Skew="0"
MODE="th" #th to mesuare throughput, rs to mesuare response time
#Arp_Delay_Base=0; #400000000
Number_of_Operations=400000;
Use_Case="kv";
Bin_Add="HYBRID"
User_kernel_add="hybrid_YCSB_kv_store_zipf"
...
for node_number_counter in $( seq $NUM_Nodes $NUM_Nodes ); do
    ...
    #------------------------------------------reset board-------------------------------------------
        ...
        for n in $( seq 0 $((node_number_counter-=1)) ); do
            ...
            printf "ssh jsaber@pc${Node_Array[n]}.cloudlab.umass.edu 'source /opt/xilinx/xrt/setup.sh; ./host/host1 network.xclbin $node_number_counter $h $Operations_Each_Node $p $MODE $Assign_FPGA $Mem_IP $Skew > /users/jsaber/host${h}-${node_number_counter}-${p}.log '\n";
            ssh jsaber@pc${Node_Array[n]}.cloudlab.umass.edu "source /opt/xilinx/xrt/setup.sh; ./host/host1 network.xclbin $node_number_counter $h $Operations_Each_Node $p $MODE $Assign_FPGA $Mem_IP $Skew > /users/jsaber/host${h}-${node_number_counter}-${p}-${Assign_FPGA}-s${Skew}.log "&
